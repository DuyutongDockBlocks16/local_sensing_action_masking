...
from .env_core import Direction, ItemType
...
_ITEM_COLOR_1 = _YELLOW
_ITEM_COLOR_2 = _GREEN
...
class Viewer(object):
    ...
    def _draw_items(self, env):
        ...
        for item in env.items:
            ...
            if item.item_type == ItemType.TYPE_1:
                item_color = _ITEM_COLOR_1
            elif item.item_type == ItemType.TYPE_2:
                item_color = _ITEM_COLOR_2

            padding = self.grid_size * 0.33
            item_size = self.grid_size - 2 * padding
            ...
            batch.add(
                ...
                (
                    "v2f",
                    (
                        (self.grid_size + 1) * x + padding + 1,  # TL - X
                        (self.grid_size + 1) * y + padding + 1,  # TL - Y
                        (self.grid_size + 1) * x + padding + item_size + 1,  # TR - X
                        (self.grid_size + 1) * y + padding + 1,  # TR - Y
                        (self.grid_size + 1) * x + padding + item_size + 1,  # BR - X
                        (self.grid_size + 1) * y + padding + item_size + 1,  # BR - Y
                        (self.grid_size + 1) * x + padding + 1,  # BL - X
                        (self.grid_size + 1) * y + padding + item_size + 1,  # BL - Y
                    ),
                ),
                ("c3B", 4 * item_color),
            )
        ...
        for wall in env.walls:
            ...
            batch.add(
                ...
                (
                    "v2f",
                    (
                        (self.grid_size + 1) * x + _WALL_PADDING + 1,  # TL - X
    ...
    def _draw_goals(self, env):
        ...
        for goal in env.goals:
            x, y = goal.location[0], goal.location[1]
            y = self.rows - y - 1  # pyglet rendering is reversed
            batch.add(
                4,
                gl.GL_QUADS,
        ...
        for goal in env.goals:
            x, y = goal.location[0], goal.location[1]
            accepted_item_type = goal.accepted_item_type
            if accepted_item_type == ItemType.TYPE_1:
                label_color = _ITEM_COLOR_1
            elif accepted_item_type == ItemType.TYPE_2:
                label_color = _ITEM_COLOR_2
            y = self.rows - y - 1
    ...
    def _draw_agents(self, env):
        ...
        for agent in env.agents:
            ...
            draw_color = _AGENT_LOADED_COLOR if agent.carrying_item else _AGENT_COLOR
