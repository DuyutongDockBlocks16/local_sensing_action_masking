...
class EnvRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                obs, rewards, dones, truncated, infos = self.envs.step(actions_env)
                ...
                if self.buffer.available_actions is not None:
                    ...
                    if isinstance(infos, dict) and "action_masks" in infos:
                        available_actions = np.array(infos["action_masks"])
                    elif isinstance(infos, list) and len(infos) > 0:
                        for env_idx, info in enumerate(infos):
                            if "action_masks" in info:
                                available_actions[env_idx] = np.array(info["action_masks"])
    ...
    def warmup(self):
        # reset env
        obs, infos = self.envs.reset()  # shape = [env_num, agent_num, obs_dim]
    ...
    def render(self):
        ...
        for episode in range(self.all_args.render_episodes):
            ...
            for step in range(self.episode_length):
                ...
                # Obser reward and next obs
                obs, rewards, dones, truncated ,infos = envs.step(actions_env)
